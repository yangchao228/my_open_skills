...
The watcher resumes independently with `./scripts/clawhub-watch-release.sh <slug> --until public|verified`. It records transient receipts under the ignored `.clawhub-release-state/` directory and reports only state changes. The exact-version install gate uses ClawHub's version file manifest, requires both repository and installed SHA-256 values to match every registry source file, and rejects undeclared installed source paths while ignoring ClawHub-generated metadata and Skill Cards. Run `./scripts/test-clawhub-installed-source.sh` to cover a repository-only omitted file, an installed hash mismatch, and an undeclared installed source path. `./scripts/clawhub-reconcile-releases.sh` promotes submitted releases to public and public releases to verified; the scheduled GitHub Action invokes it every 15 minutes and never uploads a new skill.
